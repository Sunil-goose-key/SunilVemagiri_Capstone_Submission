...
| Route | Deterministic function: `must_not_auto_respond` → escalate; else confidence ≥ threshold AND retrieval passed relevance floor AND guardrails pass → auto-respond; else → escalate | Same input always yields same output (A5); threshold derived from Day 3 precision/recall data, not the 0.80 default left unexamined |
