...
| Routing determinism (A5) | — | Same input → same output | **Fails under a cold cache.** A pre-submission clean-room test (fresh clone, no prior cache) reproduced 76.2%/23.8%/76.25% (FCR/escalation/classification accuracy) against this table's 80.0%/20.0%/81.25% — retrieval stayed byte-identical, narrowing the cause to the free-tier model not honoring `temperature=0`/`seed=0` on live calls, not to this project's routing code. Every earlier "verified reproducible" claim for A5 was actually testing a warm cache, not genuine determinism; the hidden grading set will always be cold. Full detail: revision log §4f, report §7 sixth caveat. | Clean-room test (fresh clone, `git log`/`pytest`/harness all run from scratch) |
